...
from domain_model_med.resnet_monai import ResNetFeatures
...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'EET_Med_ResNet50',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.EET_Med_ResNet50.N_CTX}
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.EET_Med_ResNet50.N_CTX
        ctx_init = cfg.TRAINER.EET_Med_ResNet50.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        # Default is 1, which is compound shallow prompting
        assert cfg.TRAINER.EET_Med_ResNet50.PROMPT_DEPTH >= 1, "For EET_Med_ResNet50, PROMPT_DEPTH should be >= 1"
        self.compound_prompts_depth = cfg.TRAINER.EET_Med_ResNet50.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
        ...
        print('EET_Med_ResNet50 design: Multi-modal Prompt Learning')
        print(f'Initial context: "{prompt_prefix}"')
        print(f"Number of EET_Med_ResNet50 context words (tokens): {n_ctx}")
        ...
        # Minimum can be 1, which defaults to shallow EET_Med_ResNet50
        # compound prompts
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.domain_model.load_state_dict(new_state_dict)
    ...
    def forward(self, image, label=None):
        ...
        with torch.no_grad():
            image_2d_gray = image.mean(dim=1, keepdim=True)
            image_3d_input = image_2d_gray.unsqueeze(2)
            list_of_features = self.domain_model(image_3d_input)
            domain_out_feature = list_of_features[1:]
            domain_out_feature[0] = domain_out_feature[0].squeeze(2).type(self.dtype)
            domain_out_feature[1] = domain_out_feature[1].squeeze(2).type(self.dtype)
            domain_out_feature[2] = domain_out_feature[2].squeeze(2).type(self.dtype)
            domain_out_feature[3] = domain_out_feature[3].squeeze(2).type(self.dtype)

        prompts, shared_ctx, deep_compound_prompts_text, deep_compound_prompts_vision = self.prompt_learner(domain_out_feature[0].type(self.dtype))
        
        image_features = self.image_encoder(image.type(self.dtype), shared_ctx, deep_compound_prompts_vision, domain_out_feature)
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        
        logits = []
        for pts_i, imf_i in zip(prompts, image_features.type(self.dtype)):
            text_features = self.text_encoder(pts_i, tokenized_prompts,deep_compound_prompts_text, domain_out_feature)
...
class EET_Med_ResNet50(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.EET_Med_ResNet50.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.EET_Med_ResNet50.PREC == "fp32" or cfg.TRAINER.EET_Med_ResNet50.PREC == "amp":
        ...
        self.scaler = GradScaler() if cfg.TRAINER.EET_Med_ResNet50.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.EET_Med_ResNet50.PREC
        if prec == "amp":
            with autocast():
